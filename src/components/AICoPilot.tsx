    ...
    <AnimatePresence>
      {isOpen && (
        <motion.div
          ...
          className="fixed top-16 right-0 bottom-0 z-30 w-full sm:w-96 panel-card bg-[#050b06] shadow-2xl backdrop-blur-md flex flex-col justify-between"
          id="ai-copilot-panel"
        >
          {/* Header */}
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-4 custom-scrollbar">
            {messages.map((msg, idx) => (
              ...
              >
                ...
                <div className={`rounded panel-card px-3 py-2 text-xs font-sans leading-relaxed max-w-[80%] ${
                  msg.sender === "user"
                    ? "bg-white/5 text-white"
                    : "bg-[#07140b]/65 text-gray-300"
                }`}>
          ...
          <div className="p-4 border-t border-white/5 bg-black/60">
            ...
            <form onSubmit={handleSend} className="relative flex items-center">
              <input
                ...
                disabled={isLoading}
                className="w-full rounded panel-card bg-white/5 py-2 pr-12 pl-3 text-xs font-sans text-white placeholder-gray-500 focus:outline-none focus:ring-emerald-500/20"
              />
              <button
                type="submit"
